...
const getBlockStyles = (props: {
  ...
  return `
    background: ${props.backgroundColor || '#24292e'} !important;
    padding: 8px;
  `;
};
...
export const CopyText = styled(Typography.Text)<{ $hasVScrollbar: boolean }>`
  ...
  font-size: 0;
  button {
    background: rgba(36, 41, 46, 0.9) !important;
...
export const createCodeBlock = (HighlightRules: any) => {
  return function CodeBlock(props: BaseProps) {
    ...
    useEffect(() => {
      const { cssText } = ace.require('ace/theme/github_dark');
    ...
    return (
      <Block
        className="ace_editor ace-github-dark adm_code-block"
        maxHeight={maxHeight}
        inline={inline}
