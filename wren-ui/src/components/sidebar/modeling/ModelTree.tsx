    ...
    useTriggerDataSourceDetectionMutation({
      ...
      onCompleted: async (data) => {
        if (data.triggerDataSourceDetection) {
          message.warning('Alteração no esquema detectada.');
        } else {
          message.success('Nenhuma alteração no esquema.');
        }
    ...
    useResolveSchemaChangeMutation({
      ...
      onCompleted: async (_, options) => {
        ...
        if (type === SchemaChangeType.DELETED_TABLES) {
          message.success('Tabela de origem excluída resolvida com sucesso.');
        } else if (type === SchemaChangeType.DELETED_COLUMNS) {
          message.success('Coluna de origem excluída resolvida com sucesso.');
        }
          ...
          <ReloadOutlined
            ...
            title={
              schemaChangeData?.schemaChange.lastSchemaChangeTime
                ? `Última atualização ${getRelativeTime(schemaChangeData?.schemaChange.lastSchemaChangeTime)}`
                : ''
            }
      ...
      getModelGroupNode({
        ...
        appendSlot: hasSchemaChange && (
          <span className="adm-actionIcon mx-2" onClick={onOpenSchemaChange}>
            <WarningOutlined
              className="orange-5"
              title="Revisar impactos das alterações no esquema"
            />
          </span>
        ),
