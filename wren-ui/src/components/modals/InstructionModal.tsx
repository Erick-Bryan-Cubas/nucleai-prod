...
export default function InstructionModal(props: Props) {
  ...
  return (
    <Modal
      title={`${isCreateMode ? 'Adicionar' : 'Atualizar'} instrução`}
      centered
      closable
    ...
    >
      <Form form={form} preserve={false} layout="vertical">
        ...
        >
          <Input.TextArea
            autoFocus
            placeholder="Insira uma regra que o Wren AI deve seguir ao gerar consultas SQL."
            maxLength={1000}
            rows={3}
            showCount
          />
        ...
        <Form.Item
          ...
          extra={
            <>
              Escolha se esta instrução se aplica a{' '}
              <span style={{ color: 'var(--gray-5)' }}>todas as consultas</span> ou{' '}
              <span style={{ color: 'var(--gray-5)' }}>
                apenas quando perguntas similares do usuário forem detectadas
              </span>
              .
            </>
          }
        >
          <Radio.Group>
            <Radio.Button value={true}>
              Global (aplica-se a todas as perguntas)
            </Radio.Button>
            <Radio.Button value={false}>
              Correspondência a perguntas específicas
            </Radio.Button>
          </Radio.Group>
        </Form.Item>
        {!isDefault && (
          <Form.Item
            label="Perguntas correspondentes"
            required
            extra="O Wren AI irá corresponder as consultas do usuário com base em similaridade e aplicará esta instrução quando relevante."
          >
                  ...
                  {fields.map(({ key, name, ...restField }) => (
                    <Row key={key} wrap={false} gutter={8} className="my-2">
                      <Col flex="1 0">
                        ...
                        >
                          <Input
                            placeholder="Insira uma pergunta de exemplo que deve acionar esta instrução."
                            maxLength={100}
                            showCount
                          />
                        </Form.Item>
                      ...
                      <Col flex="none" className="p-1">
                        <Button
                          ...
                          icon={<DeleteOutlined />}
                          size="small"
                          style={{ border: 'none', background: 'rgba(255,255,255,0.04)', color: 'var(--gray-5)' }}
                        />
                      </Col>
                    </Row>
                  ))}
