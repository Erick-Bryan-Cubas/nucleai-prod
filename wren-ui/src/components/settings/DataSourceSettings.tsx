...
const SampleDatasetPanel = (props: Props) => {
  ...
  const onSelect = (name: SampleDatasetName) => {
    ...
    if (!isCurrentTemplate) {
      ...
      Modal.confirm({
        title: `Tem certeza que deseja mudar para o dataset "${template.label}"?`,
  ...
  return (
    <>
      <div className="mb-2">Alterar dataset de exemplo</div>
      ...
      <div className="gray-6 mt-1">
        Atenção: escolher outro dataset de exemplo excluirá todos os registros de
        threads na Página Inicial.
      </div>
    </>
  );
};
...
const DataSourcePanel = (props: Props) => {
  ...
  const [updateDataSource, { loading, error }] = useUpdateDataSourceMutation({
    ...
    onCompleted: async () => {
      refetchSettings();
      message.success('Fonte de dados atualizada com sucesso.');
    },
  });
