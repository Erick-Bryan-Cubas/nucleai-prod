...
export default function SQLPairDrawer(props: Props) {
  ...
  return (
    <Drawer
      ...
      destroyOnClose
      onClose={onClose}
      title="Ver par Pergunta-SQL"
      visible={visible}
      width={760}
    >
      <div className="mb-6">
        <Typography.Text className="gray-7 mb-2">Pergunta</Typography.Text>
        <div>{defaultValue?.question || '-'}</div>
      </div>
      <div className="mb-6">
        <Typography.Text className="gray-7 mb-2">Instrução SQL</Typography.Text>
        <SQLCodeBlock
