...
import ScatterProperties from '@/components/chart/properties/ScatterProperties';
import HeatmapProperties from '@/components/chart/properties/HeatmapProperties';
...
const getDynamicProperties = (chartType: ChartType) => {
  const propertiesMap = {
    ...
    [ChartType.SCATTER]: ScatterProperties,
    [ChartType.HEATMAP]: HeatmapProperties,
  };
...
export default function ChartAnswer(props: AnswerResultProps) {
  ...
  const [createDashboardItem] = useCreateDashboardItemMutation({
    onError: (error) => console.error(error),
    onCompleted: () => {
      message.success('Gráfico fixado no dashboard com sucesso.');
    },
  });
  ...
  // When chartType changes, the DynamicProperties component swaps out.
  // Ant Design Form loses the chartType field value on remount, so we
  // restore it explicitly before capturing newValues.
  const onFormChange = () => {
  ...
  const onReload = () => {
    Modal.confirm({
      title: 'Tem certeza que deseja regenerar o gráfico?',
      onOk: onRegenerate,
    });
  };
  ...
  const onPin = () => {
    Modal.confirm({
      title: 'Tem certeza que deseja fixar este gráfico no dashboard?',
      okText: 'Salvar',
      onOk: async () =>
  ...
  return (
    <StyledSkeleton
      ...
      title={false}
    >
      <div className="text-md p-6" style={{ color: 'var(--gray-3)' }}>
        {chartDetail?.description}
        {chartSpec ? (
          <ChartWrapper
            className={clsx(
              'rounded mt-4 pb-3 overflow-hidden',
