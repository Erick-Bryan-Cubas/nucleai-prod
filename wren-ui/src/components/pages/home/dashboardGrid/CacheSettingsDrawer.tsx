...
const getDayOfWeekText = (day: CacheScheduleDayEnum) => {
  return (
    {
      [CacheScheduleDayEnum.MON]: 'Segunda-feira',
      [CacheScheduleDayEnum.TUE]: 'Terça-feira',
      [CacheScheduleDayEnum.WED]: 'Quarta-feira',
      [CacheScheduleDayEnum.THU]: 'Quinta-feira',
      [CacheScheduleDayEnum.FRI]: 'Sexta-feira',
      [CacheScheduleDayEnum.SAT]: 'Sábado',
      [CacheScheduleDayEnum.SUN]: 'Domingo',
    }[day] || ''
  );
};

const getFrequencyText = (frequency: string) => {
  if (frequency === FREQUENCY.NEVER) return 'Somente atualização manual';
  ...
  return map[frequency] || capitalize(frequency);
};
...
export const getScheduleText = (schedule: Schedule): string => {
  ...
  switch (frequency) {
    case FREQUENCY.DAILY: {
      ...
      return `Cache atualiza diariamente às ${time}`;
    }
    case FREQUENCY.WEEKLY: {
      const time = convertTime(schedule);
      return `Cache atualiza toda ${getDayOfWeekText(schedule.day as CacheScheduleDayEnum)} às ${time}`;
    }
    case FREQUENCY.CUSTOM: {
      return `Cache atualiza em agendamento personalizado`;
    }
    case FREQUENCY.NEVER: {
      return 'Atualização do cache: somente manual';
    }
    default: {
      return '';
    }
    ...
    >
      <Form form={form} layout="vertical">
        <Form.Item
          ...
          extra="Habilite o cache para acelerar o carregamento do dashboard reutilizando resultados recentes. Escolha um agendamento de atualização abaixo."
        >
          <Switch />
        </Form.Item>
...
function Schedule() {
  ...
  return (
    <>
      <Divider style={{ color: 'var(--gray-6)', borderColor: 'rgba(255,255,255,0.08)' }} className="text-sm">Configurações de atualização</Divider>
      <Form.Item label="Frequência" name={['schedule', 'frequency']}>
        <Select
      ...
      {frequency === FREQUENCY.CUSTOM && (
        <Form.Item
          ...
          extra="A expressão Cron será executada no fuso horário UTC (ex: '0 0 * * *' para diariamente à meia-noite UTC)"
        >
