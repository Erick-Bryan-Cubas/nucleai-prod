...
const REQUIRED_TYPE_RULE = [
  { required: true, whitespace: true, message: 'Type is required' },
];
    ...
    onChange({
      [fieldsName]: value.map((item) => ({
        ...
        ...([FIELDS_NAME.FIELDS, FIELDS_NAME.NESTED_FIELDS].includes(fieldsName)
          ? { displayName: item.displayName, type: item.type }
          : {}),
      })),
    });
  };
    ...
    <>
      ...
      <div className="mb-6">
        ...
        <FieldEditableTable
          ...
          columns={[
            COLUMN.NAME,
            { ...COLUMN.ALIAS, editable: true },
            {
              ...COLUMN.TYPE,
              width: 150,
              editable: true,
              rules: REQUIRED_TYPE_RULE,
            },
            { ...COLUMN.DESCRIPTION, width: 280, editable: true },
          ]}
