...
export class AskingResolver {
  ...
  public async createThread(
    ...
    let resolvedSql: string | undefined;
    if (data.taskId) {
      ...
      resolvedQuestion = askingTask.question;
      const firstCandidate = (askingTask.response || [])[0] as
        | { sql?: string }
        | undefined;
      resolvedSql = firstCandidate?.sql;
    } else {
      // when we use recommendation questions, there's no task to track
      threadInput = data;
      resolvedQuestion = data.question;
      resolvedSql = data.sql;
    }
  ...
  // NucleAI: persist a home-question into api_history so it shows up under
  // "API → Histórico". Called from createThread and createThreadResponse so
  // every entry-point (typed text, recommendation click, follow-up) is logged.
  private async recordAskHistory(
    ...
    try {
      ...
      if (args.error) responsePayload.error = args.error;
      await ctx.apiHistoryRepository.createOne({
        id: uuidv4(),
        projectId: project.id,
        apiType: ApiType.ASK,
        threadId: args.threadId ? String(args.threadId) : undefined,
        requestPayload: { question: args.question },
        responsePayload,
        statusCode: args.statusCode,
        durationMs: Date.now() - args.startedAt,
      });
    } catch (err) {
      logger.warn(`Failed to record ASK history: ${err}`);
    }
  }
