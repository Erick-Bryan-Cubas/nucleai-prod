...
export class ModelResolver {
  ...
  public async checkModelSync(_root: any, _args: any, ctx: IContext) {
    ...
    const lastDeployHash = lastDeploy?.hash ?? null;
    const inProgressDeployment =
      await ctx.deployService.getInProgressDeployment(id);
    if (inProgressDeployment) {
      return { status: SyncStatusEnum.IN_PROGRESS, lastDeployHash };
    }
    return currentHash == lastDeployHash
      ? { status: SyncStatusEnum.SYNCRONIZED, lastDeployHash }
      : { status: SyncStatusEnum.UNSYNCRONIZED, lastDeployHash };
  }

  public async deploy(
    _root: any,
    args: { force?: boolean; skipAiReindex?: boolean },
    ctx: IContext,
    ...
    const { manifest } = await ctx.mdlService.makeCurrentModelMDL();
    const skipDefault = process.env.NUCLEAI_SKIP_REINDEX_DEFAULT === 'true';
    const skipAiReindex = args.skipAiReindex ?? skipDefault;
  ...
  private async handleUpdateNestedColumnMetadata(
    ...
    for (const col of modelNestedColumns) {
      ...
      if (
        !isNil(requestedMetadata.type) &&
        requestedMetadata.type.trim() !== ''
      ) {
        nestedColumnMetadata.type = requestedMetadata.type.trim();
      }
    ...
    if (!isEmpty(data.columns)) {
      ...
      for (const col of viewColumns) {
        ...
        if (
          ...
          col.type = requestedMetadata.type.trim();
        }
      }
