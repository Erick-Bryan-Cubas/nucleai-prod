...
export class DeployService implements IDeployService {
  ...
  public async deploy(manifest, projectId, opts: DeployOptions = {}) {
    const eventName = TelemetryEvent.MODELING_DEPLOY_MDL;
    const force = opts.force ?? false;
    // force always wins over skipAiReindex
    const skipAiReindex = !force && (opts.skipAiReindex ?? false);
    try {
      ...
      if (!force) {
        ...
        // "Adopt" branch: user asserts Qdrant embeddings are still valid for
        // the new manifest — write a SUCCESS row without calling wren-ai-service.
        // Refused on first-ever deploy (no prior SUCCESS => Qdrant is empty).
        if (skipAiReindex) {
          if (!lastDeploy) {
            const error =
              'Cannot skip re-indexing on first deploy — Qdrant has no embeddings yet.';
            logger.warn(error);
            return { status: DeployStatusEnum.FAILED, error };
          }
          await this.deployLogRepository.createOne({
            manifest,
            hash,
            projectId,
          ...
          logger.info(
            `Adopted AI index: hash=${hash} (prev=${lastDeploy.hash})`,
          );
          this.telemetry.sendEvent(eventName, { skipAiReindex: true });
          return { status: DeployStatusEnum.SUCCESS };
        }
      }
      const deployData = {
