...
export class AskingService implements IAskingService {
  ...
  public async adjustThreadResponseChart(
    ...
    // 3. put the task into background tracker — pass the requested
    // adjustment so the tracker can enforce the user's chartType when the
    // LLM tries to override it.
    this.chartAdjustmentBackgroundTracker.addTask(updatedThreadResponse, input);
