...
export interface IWrenAIAdaptor {
  ...
  /**
   * NucleAI: hot-reload wren-ai-service config (see
   * docker/ai-service-patches/config_reload.py).
   */
...
export class WrenAIAdaptor implements IWrenAIAdaptor {
  ...
  public async reloadConfig(): Promise<ReloadConfigResult> {
    try {
      const res = await axios.post(
        `${this.wrenAIBaseEndpoint}/v1/config/reload`,
        {},
        { timeout: 20000 },
      );
      return res.data as ReloadConfigResult;
    } catch (err: any) {
      // 404 == patch not loaded; surface as non-ok with a hint.
      const status = err?.response?.status;
      return {
        ok: false,
        error:
          status === 404
            ? 'reload endpoint not found — sitecustomize patch not mounted'
            : getAIServiceError(err),
      };
    }
  }
}
