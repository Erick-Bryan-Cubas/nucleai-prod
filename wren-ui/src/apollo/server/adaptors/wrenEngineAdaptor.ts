...
export class WrenEngineAdaptor implements IWrenEngineAdaptor {
  ...
  public async previewData(
    ...
    try {
      ...
      const engineMsg = err.response?.data?.message || err.message;
      // The engine returns 500 for SQLs the parser/binder cannot handle.
      // These are "LLM produced incompatible SQL", not real outages —
      // route them to INVALID_SQL_ERROR so the UI shows a helpful hint
      // instead of a generic connection-style failure.
      const isParserError =
        typeof engineMsg === 'string' &&
        /(Parser Error|Catalog Error|Binder Error|IllegalArgumentException|syntax error|does not exist|No enum constant)/i.test(
          engineMsg,
        );
      throw Errors.create(
        isParserError
          ? Errors.GeneralErrorCodes.INVALID_SQL_ERROR
          : Errors.GeneralErrorCodes.WREN_ENGINE_ERROR,
        {
          customMessage: engineMsg,
          originalError: err,
        },
      );
    }
