...
export const typeDefs = gql`
  ...
  type Query {
    ...
    # NucleAI: LLM provider settings (singleton)
    llmConfig: LLMConfig!

    # NucleAI: list models available on an Ollama host. If endpoint is
    # omitted, uses the endpoint stored in llm_config.
    listOllamaModels(endpoint: String): ListOllamaModelsResult!
  ...
  type Mutation {
    ...
    deploy(force: Boolean, skipAiReindex: Boolean): JSON!
    ...
    # NucleAI: LLM provider configuration
    updateLLMConfig(data: UpdateLLMConfigInput!): UpdateLLMConfigResult!
    testLLMConnection(
    ...
    probeOllamaEmbeddingDim(
      endpoint: String!
      model: String!
    ): ProbeEmbeddingDimResult!

    # NucleAI: delete selected API History rows. Returns the number of rows deleted.
    deleteApiHistory(ids: [String!]!): Int!
  }

  # NucleAI: LLM provider settings — exposed read-only to the UI.
  # The OpenAI API key itself is never returned; hasOpenaiApiKey indicates
