...
export default function ManageQuestionSQLPairs() {
  ...
  const [createSqlPairMutation, { loading: createSqlPairLoading }] =
    useCreateSqlPairMutation(
      getBaseOptions({
        onCompleted: () => {
          message.success('Par Pergunta-SQL criado com sucesso.');
        },
      }),
    );
  ...
  const [editSqlPairMutation, { loading: editSqlPairLoading }] =
    useUpdateSqlPairMutation(
      getBaseOptions({
        onCompleted: () => {
          message.success('Par Pergunta-SQL atualizado com sucesso.');
        },
      }),
    );
  ...
  const columns: TableColumnsType<SqlPair> = [
    ...
    },
    {
      ...
      dataIndex: 'createdAt',
      width: 130,
      render: (time) => <Text style={{ color: 'var(--gray-6)' }}>{getCompactTime(time)}</Text>,
    },
    {
      key: 'action',
      width: 64,
  ...
  return (
    <SiderLayout loading={false}>
      <PageLayout
        title={
          <>
            <FunctionOutlined className="mr-2" style={{ color: 'var(--geekblue-4)' }} />
            Gerenciar pares Pergunta-SQL
          </>
        }
        titleExtra={
          <Button
          ...
          >
            Adicionar par Pergunta-SQL
          </Button>
        }
        description={
          <>
            Nesta página, você pode gerenciar seus pares Pergunta-SQL salvos. Esses
            pares ajudam o NucleAI a aprender como sua organização escreve SQL,
            permitindo gerar consultas mais alinhadas às suas expectativas.{' '}
            <Link
              className="underline"
