...
export default function ManageInstructions() {
  ...
  const [updateInstructionMutation, { loading: updateInstructionLoading }] =
    useUpdateInstructionMutation(
      getBaseOptions({
        onCompleted: () => {
          message.success('Instrução atualizada com sucesso.');
        },
      }),
    );
  ...
  const [deleteInstructionMutation] = useDeleteInstructionMutation(
    getBaseOptions({
      onCompleted: () => {
        message.success('Instrução excluída com sucesso.');
      },
    }),
  );
  ...
  const columns: TableColumnsType<Instruction> = [
    ...
    {
      title: 'Perguntas correspondentes',
      dataIndex: 'questions',
      width: '50%',
          ...
          <StyledQuestionsBlock>
            ...
            {moreCount > 0 && (
              <div className="text-sm gray-7 pl-1">
                +{moreCount} pergunta{moreCount > 1 ? 's' : ''} adicionais
              </div>
            )}
          </StyledQuestionsBlock>
        );
  ...
  return (
    <SiderLayout loading={false}>
      <PageLayout
        ...
        description={
          <>
            Nesta página, você pode gerenciar instruções salvas que orientam o NucleAI
            na geração de consultas SQL. Essas instruções ajudam o NucleAI a
            entender seu modelo de dados e regras de negócio, melhorando a precisão
            das consultas e reduzindo a necessidade de ajustes manuais.{' '}
            <Link
