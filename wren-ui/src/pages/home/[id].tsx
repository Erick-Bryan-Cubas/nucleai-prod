...
export default function HomeThread() {
  ...
  const threadId = useMemo(() => {
    ...
    return typeof rawId === 'string' ? Number(rawId) || null : null;
  }, [router.query.id]);
  ...
  const [createViewMutation, { loading: creating }] = useCreateViewMutation({
    ...
    onCompleted: () => message.success('View criada com sucesso.'),
  });
  ...
  useEffect(() => {
    if (threadError) router.push(Path.Home);
  }, [threadError]);
  ...
  const [updateThreadResponse, { loading: threadResponseUpdating }] =
    useUpdateThreadResponseMutation({
      onError: (error) => console.error(error),
      onCompleted: (data) => {
        message.success('Instrução SQL atualizada com sucesso.');
  ...
  useEffect(() => {
    const nextResponse = threadResponseResult.data?.threadResponse;
