...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!error) return;
    message.error('Não foi possível carregar os itens do dashboard.');
  ...
  const [setDashboardSchedule] = useSetDashboardScheduleMutation({
    ...
    onCompleted: () => {
      message.success('Agendamento do dashboard atualizado com sucesso.');
    },
  ...
  const [updateDashboardItemLayouts] = useUpdateDashboardItemLayoutsMutation({
    onError: () => {
      message.error('Não foi possível atualizar o layout dos itens do dashboard.');
    },
  });
  const [deleteDashboardItem] = useDeleteDashboardItemMutation({
    onError: (error) => console.error(error),
    onCompleted: (_, query) => {
      message.success('Item do dashboard excluído com sucesso.');
